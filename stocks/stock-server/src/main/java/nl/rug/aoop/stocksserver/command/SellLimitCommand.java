...
/**
 * Models a sell-limit command.
 */
@Slf4j
public class SellLimitCommand implements Command {
    private final StockExchange stockExchange;

    /**
     * Constructor for sell-limit command.
     * @param stockExchange the stock exchange.
     */
    public SellLimitCommand(StockExchange stockExchange) {
        this.stockExchange = stockExchange;
    }

    /**
     * Executes a sell-limit by resolving order in the transaction manager.
    ...
    public synchronized void execute(Map<String, Object> options) {
        ...
        SellOrder sellOrder = new SellOrder(id,stockSymbol, nrShares,priceLimit);

        stockExchange.getTransactionManager().finalizeSellOrder(sellOrder);
    }
}
